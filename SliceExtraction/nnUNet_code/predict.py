...
from six.moves import cPickle as myPickle 
import os
# -----------------------------------------------------------------------------
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
                  ...
                  applyQC: bool = False, qcModelPath: str = ''):
    """
    ...
    # ---------------------------------- ADDED --------------------------------
    myFeatures = []
    casesProbablyContainingNoL3 = []
        
    if os.path.exists(qcModelPath):
        with open(qcModelPath, 'rb') as f:
            lrClassifier = myPickle.load(f)
    # -------------------------------------------------------------------------
    
    for preprocessed in preprocessing:
        output_filename, (d, dct) = preprocessed
        # ---------------------------------- ADDED ----------------------------
        rawPath = dct['list_of_data_files'][0]
        # ---------------------------------------------------------------------
        ...
        # ------------------------------- ADDED -------------------------------
        if applyQC:
            curSoftmax = softmax_mean[1]
            foregroundIndices = np.where(np.round(curSoftmax.flatten())==1.0)
            
            numberOfPixels = len(foregroundIndices[0])
            
            indxFilesep = [i for i in range(0,len(rawPath)) if rawPath[i]=='/']
            pat_name = rawPath.split('/')[-1].split('.')[0]
            outputPathWarning = '%s/%s_Warning.txt' %(rawPath[0:indxFilesep[-1]],pat_name)

            features = [numberOfPixels]
                      
            containsL3 = lrClassifier.predict(np.array(features).reshape(1,-1))
            
            myFeatures.append([pat_name,numberOfPixels,containsL3])

            if containsL3 == 0 or numberOfPixels==0:
                warning = 'Warning: Patient %s probably does not contain L3/L4 slice. Please check the prediction result manually.' %pat_name
                print(warning)
                f = open(outputPathWarning,"w")
                f.writelines(warning)
                f.close()  
                casesProbablyContainingNoL3.append([rawPath,numberOfPixels,containsL3])
    	#----------------------------------------------------------------------
    ...
    if applyQC:  
        ...
        outputDir = os.path.dirname(output_filenames[0])

        if len(myFeatures) != 0:
            curSoftmaxResult = outputDir + '/QualityControlResult.xls'  
        
            if os.path.isfile(curSoftmaxResult):
                table = pd.read_excel(curSoftmaxResult)
                columnNames = table.columns.values.tolist()
                existingSoftmaxResult = []
                existingSoftmaxResult.extend(pd.DataFrame(table,columns=columnNames).values.tolist())
                myFeatures += existingSoftmaxResult
       
            excelPath = '%s/QualityControlResult.xls' %os.path.dirname(output_filenames[0])
            workbook = xlsx.Workbook(excelPath)
            worksheet = workbook.add_worksheet()
            
            worksheet.write('A1','Patient Name')
            worksheet.write('B1','Number of Foreground Pixels')
            worksheet.write('C1','containsL3')

            names = [myFeatures[i][0] for i in range(0,len(myFeatures))]
            numberOfPixels = [myFeatures[i][1] for i in range(0,len(myFeatures))]           
            containsL3 = [myFeatures[i][2] for i in range(0,len(myFeatures))]
        
            worksheet.write_column('A2', names)
            worksheet.write_column('B2', numberOfPixels)
            worksheet.write_column('C2', containsL3)
                        
            workbook.close() 
            
        if len(casesProbablyContainingNoL3) != 0:
            
            curNoL3 = outputDir + '/ScansProbablyContainingNoL3.xls'             
            if os.path.isfile(curNoL3):
                table = pd.read_excel(curNoL3)
                columnNames = table.columns.values.tolist()
                existingCasesNoL3 = []
                existingCasesNoL3.extend(pd.DataFrame(table,columns=columnNames).values.tolist())
                casesProbablyContainingNoL3 += existingCasesNoL3
                     
            excelPathContainingNoL3 = '%s/ScansProbablyContainingNoL3.xls' %os.path.dirname(output_filenames[0])
            workbook = xlsx.Workbook(excelPathContainingNoL3)
            worksheet = workbook.add_worksheet()
            worksheet.write('A1','Path Raw Data')
            worksheet.write('B1','Number of Foreground Pixels')
            worksheet.write('C1','containsL3')
            
            rawPath = [casesProbablyContainingNoL3[i][0] for i in range(0,len(casesProbablyContainingNoL3))]
            numberOfPixels = [casesProbablyContainingNoL3[i][1] for i in range(0,len(casesProbablyContainingNoL3))]
            containsL3 = [casesProbablyContainingNoL3[i][2] for i in range(0,len(casesProbablyContainingNoL3))]

            worksheet.write_column('A2', rawPath)
            worksheet.write_column('B2', numberOfPixels)
            worksheet.write_column('C2', containsL3)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        segmentation_export_kwargs: dict = None,
                        applyQC: bool = False, qcModelPath: str = ''): # added by MT: applyQC, qcModelPath
    """
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             applyQC=applyQC, qcModelPath = qcModelPath)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = True
        else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--applyQC', default=False, required=False,action='store_true',
                      help='Use it if you want to apply quality control')
    parser.add_argument('--qcModelPath', default='', required=False,type=str,
                      help='Path to model.pkl for quality control (if you want to apply QC, set applyQC to True '
                           'and also set --save_npz (because you need softmax!)')
    # -------------------------------------------------------------------------
    ...
    # ------------------------- ADDED (FOR QUALITY CONTROL) ----------------
    # added: applyQC and qcModelPath (original code is comment out above)
    predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
                        num_threads_nifti_save, lowres_segmentations, part_id, num_parts, tta, mixed_precision=not args.disable_mixed_precision,
                        overwrite_existing=overwrite, mode=mode, overwrite_all_in_gpu=all_in_gpu, step_size=step_size,
                        applyQC = applyQC, qcModelPath = qcModelPath)
